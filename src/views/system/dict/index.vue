    ...
    <ele-card class="search-card" :body-style="{ padding: '16px 20px' }">
      <div class="search-row">
        <div class="search-item">
          <span class="search-label">字典类型</span>
          <el-input
            v-model="query.name"
            clearable
            placeholder="请输入字典类型"
...
<script setup>
  ...
  import { deleteDictType, listDictTypes } from '@/api/dict';
  import DictItemsDrawer from './components/dict-items-drawer.vue';
  import DictTypeDrawer from './components/dict-type-drawer.vue';

  const tableRef = ref(null);
  const detailVisible = ref(false);
  const editVisible = ref(false);
  const query = reactive({ name: '' });
  const current = reactive({ code: '', name: '' });
  const editing = reactive({ id: null, name: '' });

  const columns = [
    { prop: 'index', label: '序号', width: 70, align: 'center' },
    { prop: 'name', label: '字典类型', minWidth: 160, slot: 'name' },
    { prop: 'itemCount', label: '字典项数量', width: 140 },
    { columnKey: 'action', label: '操作', width: 140, slot: 'action' }
  ];

  const datasource = async ({ page, limit, where }) => {
    const all = await listDictTypes();
  ...
  const search = () => {
    tableRef.value?.reload?.({ page: 1, where: { name: query.name } });
  };
  ...
  const handleDelete = (row) => {
    ElMessageBox.confirm(`确定删除「${row.name}」吗？其下字典项会一并删除。`, '系统提示', {
      type: 'warning',
      draggable: true
    })
      .then(async () => {
